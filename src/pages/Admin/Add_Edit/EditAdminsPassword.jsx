...
const EditPasswordAdmin = () => {
  const admin = localStorage.getItem("adminToEdit");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.az-tenders.com/admin/update-paswd/${admin}`,
        formData,
        config
      );
      ...
      if (response.status === 200) {
        ...
        navigate("/admin/administrateurs"); // Rediriger vers la page Admin après la mise à jour
      }
    } catch (error) {
      setErrorMessage("Erreur lors de la mise à jour du mot de passe");
      console.error("Erreur:", error);
      Swal.fire("", "Erreur lors de la mise à jour du mot de passe", "error");
    }
  };
