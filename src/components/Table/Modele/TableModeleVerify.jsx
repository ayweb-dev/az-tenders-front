...
import CheckUserSubscription from "../CheckComponent";
            ...
            {headers.map((header) => (
              <td key={header.column} className="px-4 py-2 text-gray-700">
                {header.column === "abonne" ? (
                  // Utiliser le composant pour vérifier l'abonnement
                  <CheckUserSubscription userId={item._id} />
                ) : (
                  item[header.column]
                )}
              </td>
...
const Table = ({ headers, data, isLoading, loadingTag, delete_func }) => {
